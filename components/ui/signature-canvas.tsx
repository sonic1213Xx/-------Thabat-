...
export function SignatureCanvas({ initialSignature, onCancel, onSave, showDefaultOption = false }: { initialSignature?: string | null; onCancel: () => void; onSave: (signature: string, saveAsDefault?: boolean) => void; showDefaultOption?: boolean }) {
  ...
  const start = (event: React.PointerEvent<HTMLCanvasElement>) => { const canvas = canvasRef.current!; const context = canvas.getContext('2d')!; const { x, y } = point(event); drawingRef.current = true; canvas.setPointerCapture(event.pointerId); context.lineWidth = 3; context.lineCap = 'round'; context.lineJoin = 'round'; context.strokeStyle = inkColor; context.beginPath(); context.moveTo(x, y) }
  const move = (event: React.PointerEvent<HTMLCanvasElement>) => { if (!drawingRef.current) return; const context = canvasRef.current!.getContext('2d')!; const { x, y } = point(event); context.lineWidth = 3; context.lineCap = 'round'; context.lineJoin = 'round'; context.strokeStyle = inkColor; context.lineTo(x, y); context.stroke(); setEmpty(false) }
