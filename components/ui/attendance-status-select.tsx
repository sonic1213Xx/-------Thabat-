...
export function AttendanceStatusSelect({ value, onValueChange, options, english = false, className = '', variant = 'dropdown' }: Props) {
  ...
  if (variant === 'buttons') return <div className={`flex w-full flex-row flex-nowrap items-center gap-1 overflow-visible ${className}`} role="group" aria-label={english ? 'Attendance status' : 'الحالة'}>
    {options.map((option) => {
        ...
        : option.value === 'ABSENT_UNEXCUSED'
          ...
          : option.value === 'ABSENT_EXCUSED'
            ...
            : option.value === 'ESCAPED'
                  ...
                  return <button key={option.value} type="button" aria-pressed={value === option.value} onClick={() => onValueChange(option.value)} className={`inline-flex min-h-10 min-w-0 shrink items-center justify-center gap-1.5 whitespace-nowrap rounded-lg border px-3 py-2 text-sm font-bold transition-colors duration-150 ${value === option.value ? activeClasses : 'border-transparent bg-gray-100 text-gray-500 hover:border-slate-300 hover:bg-gray-200 dark:bg-zinc-800 dark:text-gray-400 dark:hover:border-zinc-600 dark:hover:bg-zinc-700'}`}><Icon className="h-4 w-4 shrink-0" />{optionLabel}</button>
    })}
  </div>
