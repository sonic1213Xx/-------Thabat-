...
export default function LoginPage() {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (createMode) {
      ...
      if (trimmedId === '10' || getProfiles().some((profile) => profile.id === trimmedId)) {
        setError(locale === 'ar' ? 'هذا الرقم مستخدم بالفعل.' : 'This ID is already in use.')
        return
      }
      const user = { id: trimmedId, name: trimmedName, role: 'TEACHER' as const }
      saveProfile({ ...user, password, createdAt: new Date().toISOString(), lastActivity: locale === 'ar' ? 'لم يسجل الدخول بعد' : 'Not logged in yet', assigned_divisions: [] })
      enterDashboard(user)
      return
    }
