...
type AuthenticatedUser = NonNullable<ReturnType<typeof authenticate>> & { assigned_divisions?: string[]; subjectsTaught?: string[] }
...
export default function LoginPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (createMode) {
      ...
      const response = await fetch('/api/auth/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: trimmedId, name: trimmedName, password, divisions: selectedDivisions, subjectsTaught: subjectsTaught.split(',').map((subject) => subject.trim()).filter(Boolean) }) })
      ...
      saveProfile({ ...result.data, password, createdAt: new Date().toISOString(), lastActivity: locale === 'ar' ? 'لم يسجل الدخول بعد' : 'Not logged in yet', assigned_divisions: result.data.assigned_divisions ?? [], subjectsTaught: result.data.subjectsTaught ?? [] })
    ...
    try {
      ...
      if (result.data) saveProfile({ ...result.data, password, createdAt: new Date().toISOString(), lastActivity: new Date().toISOString(), assigned_divisions: result.data.assigned_divisions ?? [], subjectsTaught: result.data.subjectsTaught ?? [] })
  ...
  return <main className={`relative isolate flex min-h-screen items-center justify-center overflow-hidden bg-transparent px-4 py-10 ${isEntering ? 'login-page-entering' : ''}`} dir={locale === 'ar' ? 'rtl' : 'ltr'}>
    ...
    <section className={`relative z-10 bg-slate-950/75 backdrop-blur-xl border border-emerald-500/20 shadow-2xl shadow-emerald-950/50 rounded-2xl p-8 w-full max-w-md transition-[box-shadow,border-color] duration-500 ${createMode ? 'login-card-create' : ''}`}>
      ...
      <form key={createMode ? 'create-profile' : 'sign-in'} onSubmit={submit} className="login-form-mode space-y-5">
        ...
        {createMode && <>
          <label className="block"><span className="mb-2 block text-xs font-medium text-slate-300">{locale === 'ar' ? 'المواد التي تدرسها' : 'Subjects taught'}</span><input value={subjectsTaught} onChange={(event) => setSubjectsTaught(event.target.value)} placeholder={locale === 'ar' ? 'افصل المواد بفاصلة' : 'Separate subjects with commas'} className="w-full rounded-xl border border-slate-800 bg-slate-900/80 px-4 py-3 text-sm text-white outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" /></label>
          <fieldset className="rounded-xl border border-slate-800 p-3"><legend className="px-1 text-xs font-medium text-slate-300">{locale === 'ar' ? 'الشعب التي تدرسها' : 'Divisions taught'}</legend><div className="flex flex-wrap gap-2">{divisions.map((code) => <label key={code} className={`cursor-pointer rounded-lg border px-3 py-2 text-xs ${selectedDivisions.includes(code) ? 'border-emerald-500 bg-emerald-600 text-white' : 'border-slate-700 text-slate-300'}`}><input type="checkbox" className="sr-only" checked={selectedDivisions.includes(code)} onChange={() => setSelectedDivisions((current) => current.includes(code) ? current.filter((item) => item !== code) : [...current, code])} />{code}</label>)}</div></fieldset>
        </>}
