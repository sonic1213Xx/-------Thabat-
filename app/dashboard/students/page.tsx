...
import { AlertCircle, BookOpen, ChevronLeft, ChevronRight, Download, Edit3, Eye, FileText, Filter, Loader2, Plus, Search, Trash2, Upload, UserCog, X } from 'lucide-react'
...
export default function StudentsPage() {
  ...
  const [gradebookExportPeriod, setGradebookExportPeriod] = useState<'period1' | 'period2' | 'both'>('both')
  const [isExporting, setIsExporting] = useState(false)
  const exportEmptyGradebookTemplates = (divisions: string[], period = gradebookExportPeriod) =>
    runExport(() => exportGradebookTemplates(divisions, period), showToast, updateToast).finally(() => setIsExporting(false))
  ...
  useEffect(() => {
    const syncDivisions = async () => {
      try {
        const json = await fetchCached<{ data?: Array<{ id: string; code: string; name: string }> }>('dashboard:divisions', '/api/divisions')
    ...
    async function loadStudents() {
      ...
      await withMinimumDelay(async () => {
        try {
          ...
          const json = await fetchCached<{ data?: StudentRecord[] }>(`dashboard:students:${division}`, url)
  ...
  const canInspectTeachers = hasPermission(currentRole, 'gradebooks', 'read') && !isTeacher
  const canDownloadGradebookTemplates = hasPermission(currentRole, 'gradebooks', 'read')
  ...
  const assignedDivisionOptions = currentRole === 'TEACHER'
    ...
    : divisionOptions
  const downloadableDivisionOptions = isTeacher ? Array.from(new Set(assignedDivisionOptions)) : divisionOptions
  ...
  const refreshStudents = async (selectedDivision = division) => {
    ...
    const json = await fetchCached<{ data?: StudentRecord[] }>(`dashboard:students:${selectedDivision}`, url)
    setStudents(json.data ?? [])
  }
  ...
  return (
    <div className="space-y-6 min-h-[320px]">
      ...
      {(canDownloadGradebookTemplates || isTeacher) && <div className="flex flex-wrap items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50/50 p-3 dark:border-emerald-900/50 dark:bg-emerald-950/20"><span className="text-sm font-semibold text-slate-700 dark:text-slate-200">{locale === 'ar' ? 'فترة التصدير:' : 'Export term:'}</span>{([['period1', locale === 'ar' ? 'الفترة 1' : 'Term 1'], ['period2', locale === 'ar' ? 'الفترة 2' : 'Term 2'], ['both', locale === 'ar' ? 'الفترتان' : 'Both terms']] as const).map(([period, label]) => <button key={period} type="button" onClick={() => setGradebookExportPeriod(period)} className={`rounded-lg px-3 py-2 text-sm font-semibold transition ${gradebookExportPeriod === period ? 'bg-emerald-600 text-white' : 'border border-emerald-200 bg-white text-emerald-700 hover:bg-emerald-50 dark:border-emerald-900 dark:bg-slate-900 dark:text-emerald-300'}`}>{label}</button>)}{isTeacher && <button type="button" onClick={() => { setTemplateDivisions(downloadableDivisionOptions); setGradebookExportOpen(true) }} className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-3 py-2 text-sm font-semibold text-white"><FileText className="h-4 w-4" />{locale === 'ar' ? 'تحميل القالب' : 'Download template'}</button>}</div>}
