...
import {
  GradebookTable,
  type GradebookRow,
} from "@/components/gradebook-table";
import {
  exportEmptyGradebookTemplates as exportGradebookTemplates,
...
export default function StudentsPage() {
  ...
  const getGradebookExportMetadata = (): GradebookExportMetadata => {
    const session = getSession();
    const profiles = getProfiles();
    const currentProfile = profiles.find(
      (profile) => profile.id === session?.id,
    );
    const principalProfile = profiles.find(
    ...
    try {
      schoolName =
        (
          JSON.parse(localStorage.getItem("thabat-settings") ?? "{}") as {
            schoolName?: string;
          }
        ).schoolName ?? "";
    } catch {
  ...
  const exportEmptyGradebookTemplates = (
    divisions: string[],
    period = gradebookExportPeriod,
  ) =>
    runExport(
      () =>
        exportGradebookTemplates(
          divisions,
          period,
          getGradebookExportMetadata(),
        ),
      showToast,
      updateToast,
  ...
  >([]);
  const [availableDivisionCodes, setAvailableDivisionCodes] = useState<
    string[]
  >([]);
  ...
  const [gradebookExportType, setGradebookExportType] = useState<
    "EXCEL" | "PDF"
  >("EXCEL");
  ...
  const [transferStudent, setTransferStudent] = useState<StudentRecord | null>(
    null,
  );
  ...
  const bulkEditStudents = (importResult?.students ?? selectedStudents).map(
    (student) => ({
      id: student.id,
      fullName: student.fullName,
      academicId: student.academicId ?? null,
      gpa: student.gpa ?? null,
      parentPhone: student.parentPhone ?? null,
      nationalId: student.nationalId ?? null,
      divisionCode: student.divisionCode ?? null,
    }),
  );
  ...
  useEffect(() => {
    ...
    setTeachingAssignments(
      currentProfile?.teachingAssignments ??
        (currentProfile?.subject
          ? [
              {
                id: `assignment-${currentProfile.id}`,
                subject: currentProfile.subject,
                gradeLevel: currentProfile.gradeLevel ?? null,
                divisions: currentProfile.assigned_divisions ?? [],
                attendance: true,
                gradebook: true,
              },
            ]
  ...
  useEffect(() => {
    const syncDivisions = async () => {
      try {
        ...
        setAvailableDivisionCodes(
          nextDivisions.map((item: { code: string }) => item.code),
        );
        setTemplateDivisions((current) =>
          current.length
            ? current.filter((code) =>
                nextDivisions.some(
      ...
      await withMinimumDelay(async () => {
        try {
          ...
          const url =
            division === "all"
              ? "/api/students"
              : `/api/students?division=${division}`;
  ...
  const canEditStudents = currentRole
    ...
    : false;
  const canDeleteStudents = currentRole
    ? can(currentRole, "can_delete_records")
    : false;
  const gradebookRows: GradebookRow[] = visibleStudents.map((student) => ({
    id: student.id,
    fullName: student.fullName,
  ...
  const assignedDivisionOptions = divisionOptions;
  const downloadableDivisionOptions = isTeacher
    ? Array.from(new Set(assignedDivisionOptions))
    : divisionOptions;
      ...
      teachingAssignments
        .filter(
          (assignment) =>
            ...
            (!division ||
              assignment.divisions.length === 0 ||
              assignment.divisions.includes(division)),
        )
  ...
  const refreshStudents = async (selectedDivision = division) => {
    ...
    const url =
      selectedDivision === "all"
        ? "/api/students"
        : `/api/students?division=${selectedDivision}`;
    invalidateCached(`dashboard:students:${selectedDivision}`);
    const json = await fetchCached<{ data?: StudentRecord[] }>(
      `dashboard:students:${selectedDivision}`,
      url,
    );
    setStudents(json.data ?? []);
  };
  ...
  const handleAddStudentClick = () => {
    ...
    if (divisionOptions.length > 0 && !newStudent.divisionCode) {
      setNewStudent((current) => ({
        ...current,
        divisionCode: divisionOptions[0],
      }));
    }
  ...
  const deleteStudent = async () => {
    ...
    setDeleteBusy(true);
    const response = await fetch(`/api/students?id=${deletingStudent.id}`, {
      method: "DELETE",
    });
    if (response.ok) {
      setStudents((current) =>
        current.filter((student) => student.id !== deletingStudent.id),
      );
  ...
  if (!hasDivisions && !isLoading) {
    return (
      <div className="space-y-6 min-h-[320px]">
        <div className="rounded-2xl border border-dashed border-emerald-300 bg-white p-10 text-center shadow-sm dark:border-emerald-700/50 dark:bg-slate-900">
          ...
          <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-5 py-3 text-sm font-medium text-white hover:bg-emerald-700"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-5 py-3 text-sm font-medium text-slate-800 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
            >
        ...
        <DivisionGuard
          open={showDivisionGuard}
          onOpenChange={handleDivisionGuardChange}
        />
                ...
                <div className="w-full">
                  ...
                  >
                    ...
                    <StyledSelect
                      ...
                      onValueChange={(value) =>
                        setNewStudent({ ...newStudent, divisionCode: value })
                      }
                      placeholder={
                        divisionOptions.length > 0
                          ? t("chooseDivisionOption")
                          : t("noAvailableDivisions")
                      }
                      options={
                        divisionOptions.length > 0
                          ...
                          : [{ value: "", label: t("noAvailableDivisions") }]
                      }
                    />
                    <button
                      type="submit"
    ...
    <div className="space-y-6 min-h-[320px]">
        ...
        <div className="flex items-center gap-3">
          {canEditStudents && (
            <>
              <button
                onClick={() => setIsImportOpen(true)}
                className="inline-flex items-center gap-2 rounded-lg bg-slate-100 px-4 py-2 text-sm text-slate-800 hover:bg-slate-200 dark:bg-slate-800 dark:text-white dark:hover:bg-slate-700"
              >
      ...
      <div className="flex flex-wrap gap-2 rounded-xl border border-slate-200 bg-white p-2 dark:border-slate-800 dark:bg-slate-900">
        <button
          ...
          className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition ${activeView === "roster" ? "bg-emerald-600 text-white" : "text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"}`}
        >
          <BookOpen className="h-4 w-4" />{" "}
          {locale === "ar" ? "كشف الطلاب الرئيسي" : "Main student roster"}
        </button>
        {isTeacher && (
          <button
            type="button"
            onClick={() => setActiveView("gradebook")}
            className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition ${activeView === "gradebook" ? "bg-emerald-600 text-white" : "text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"}`}
          >
            <FileText className="h-4 w-4" />{" "}
            {locale === "ar" ? "كشف الدرجات" : "Gradebook"}
          </button>
        )}
        {canInspectTeachers && (
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition ${activeView === "inspection" ? "bg-emerald-600 text-white" : "text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"}`}
          >
            <Eye className="h-4 w-4" />{" "}
            {locale === "ar" ? "كشوفات المعلمين" : "Teacher inspection"}
          </button>
        )}
      </div>
              ...
              <button
                ...
                onClick={async () => {
                  if (
                    !confirm(
                      locale === "ar"
                        ? `حذف ${selectedStudentIds.length} طالباً؟`
                  ...
                  if (response.ok) {
                    ...
                    window.dispatchEvent(
                      new CustomEvent("thabat-students-changed"),
                    );
                  } else
                    alert(
                      locale === "ar"
      ...
      {canInspectTeachers && (
        <section className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-border bg-card px-4 py-3 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-card-foreground">
                {locale === "ar"
                  ? "تصدير قوالب كشوف الدرجات"
                  : "Export gradebook templates"}
              </h2>
              <p className="text-xs text-card-foreground/60">
                {locale === "ar"
                  ? `تم تحديد ${templateDivisions.length} شعبة`
                  : `${templateDivisions.length} divisions selected`}
              </p>
            </div>
          </div>
          <button
            type="button"
            onClick={() => {
              setTemplateDivisions(downloadableDivisionOptions);
              setGradebookExportOpen(true);
            }}
          ...
          >
            <FileText className="h-4 w-4" />
            {locale === "ar" ? "اختيار التصدير" : "Choose export"}
          </button>
        </section>
      )}
      {gradebookExportOpen && (
        <Modal
          open={true}
          onOpenChange={setGradebookExportOpen}
          className="max-w-2xl"
        >
          <div className="space-y-5">
            <div>
              ...
              <p className="mt-1 text-sm text-card-foreground/60">
                {locale === "ar"
                  ? "اختر نوع الملف والشعب التي تريد تضمينها."
                  : "Choose a file type and the divisions to include."}
              </p>
            </div>
            <fieldset>
              <legend className="text-sm font-bold text-card-foreground">
                {locale === "ar" ? "فترة التصدير" : "Export term"}
              </legend>
              <div className="mt-2 grid grid-cols-3 gap-2 rounded-xl bg-emerald-50 p-2 dark:bg-emerald-950/30">
                {([
                  ["period1", locale === "ar" ? "الفترة 1" : "Term 1"],
                  ["period2", locale === "ar" ? "الفترة 2" : "Term 2"],
                  ["both", locale === "ar" ? "الفترتان" : "Both terms"],
                ] as const).map(([period, label]) => (
                  <button
                    key={period}
                    type="button"
                    onClick={() => setGradebookExportPeriod(period)}
                    className={`rounded-lg px-3 py-2 text-sm font-semibold transition ${gradebookExportPeriod === period ? "bg-emerald-600 text-white shadow-sm" : "text-emerald-700 hover:bg-white dark:text-emerald-300 dark:hover:bg-slate-900"}`}
                  >
                    {label}
                  </button>
                ))}
              ...
              <div className="mt-2 grid grid-cols-2 gap-3">
                {(["EXCEL", "PDF"] as const).map((type) => (
                  <label key={type} className="cursor-pointer">
                    <input
                      type="radio"
                      name="gradebook-export-type"
                      value={type}
                      checked={gradebookExportType === type}
                      onChange={() => setGradebookExportType(type)}
                      className="peer sr-only"
                    />
                    <span
                      className={`flex justify-center rounded-xl border px-4 py-3 text-sm font-bold transition peer-focus-visible:ring-2 peer-focus-visible:ring-emerald-500 ${type === "PDF" ? "border-border hover:border-red-400 peer-checked:border-red-600 peer-checked:bg-red-50 peer-checked:text-red-700 dark:peer-checked:bg-red-950/40 dark:peer-checked:text-red-300" : "border-border hover:border-emerald-400 peer-checked:border-emerald-600 peer-checked:bg-emerald-50 peer-checked:text-emerald-700 dark:peer-checked:bg-emerald-950/40 dark:peer-checked:text-emerald-300"}`}
                    >
                      {type}
                    </span>
                  </label>
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-card-foreground/60">
                  {locale === "ar"
                    ? `تم تحديد ${templateDivisions.length} من ${downloadableDivisionOptions.length}`
                    : `${templateDivisions.length} of ${downloadableDivisionOptions.length} selected`}
                </p>
              </div>
              <button
                type="button"
                onClick={() =>
                  setTemplateDivisions((current) =>
                    current.length === downloadableDivisionOptions.length
                      ? []
                      : downloadableDivisionOptions,
                  )
                }
                className="text-sm font-semibold text-emerald-700 hover:underline dark:text-emerald-400"
              >
                {templateDivisions.length === downloadableDivisionOptions.length
                  ? locale === "ar"
                    ? "إلغاء تحديد الكل"
                    : "Clear all"
                  : locale === "ar"
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {downloadableDivisionOptions.map((code) => (
                <label key={code} className="cursor-pointer">
                  <input
                    type="checkbox"
                    checked={templateDivisions.includes(code)}
                    onChange={() =>
                      setTemplateDivisions((current) =>
            ...
            <div className="flex justify-end gap-2 border-t border-border pt-4">
              <button
                type="button"
                onClick={() => setGradebookExportOpen(false)}
              ...
              <button
                type="button"
                disabled={isExporting || !templateDivisions.length}
                onClick={() => {
                  if (gradebookExportType === "PDF") {
                    notifyPdfComingSoon(showToast);
                    setGradebookExportOpen(false);
                    return;
                  }
                  setIsExporting(true);
                  void exportEmptyGradebookTemplates(templateDivisions);
                  setGradebookExportOpen(false);
                }}
                className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50 ${gradebookExportType === "PDF" ? "bg-red-600 hover:bg-red-700" : "bg-emerald-600 hover:bg-emerald-700"}`}
              >
                {isExporting ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : null}
                {locale === "ar"
                  ? `تصدير ${gradebookExportType}`
                  : `Export ${gradebookExportType}`}
              </button>
            </div>
          </div>
        </Modal>
      ...
      {activeView === "inspection" ? (
        <TeacherInspectionView
          ...
          onAssignmentsChange={updateTeacherAssignments}
        />
      ) : (
        <>
          <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div className="flex items-center gap-2">
                <Filter
                  className={`h-4 w-4 ${!division ? "text-slate-400 dark:text-slate-600" : division === "all" ? "text-slate-400 dark:text-slate-600" : "text-emerald-600 dark:text-emerald-400"}`}
                />
                <span
                  className={`${!division ? "text-slate-500 dark:text-slate-500" : division === "all" ? "text-slate-500 dark:text-slate-500" : "text-slate-900 dark:text-white font-medium"}`}
                >
                  {!division
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div className="relative min-w-64">
                  ...
                  <input
                    ...
                    placeholder={
                      locale === "ar"
                        ? "ابحث بالاسم أو الرقم الأكاديمي أو الهوية"
                        : "Search by name, academic ID, or national ID"
                    }
                    aria-label={
                      locale === "ar" ? "البحث عن طالب" : "Search students"
                    }
                ...
                <StyledSelect
                  ...
                  placeholder={t("chooseDivisionOption")}
                  options={[
                    { value: "", label: t("chooseDivisionOption") },
                    ...(currentRole === "TEACHER"
                      ? []
            ...
            division !== "all" && (
              <>
                {gradebookRows.length > 0 ? (
                  <>
                    {availableSubjects.length > 0 && (
                      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50/50 p-3 dark:border-emerald-900/50 dark:bg-emerald-950/20">
                        ...
                        <StyledSelect
                          ...
                          value={currentSubject || availableSubjects[0]}
                    ...
                    <GradebookTable
                      ...
                      students={gradebookRows}
                      allDivisionCodes={assignedDivisionOptions}
                      readOnly={false}
                    />
                  </>
                ) : (
                  <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-900/60 dark:text-slate-400">
                    {locale === "ar"
                      ? "لا يوجد طلاب في هذه الشعبة بعد."
                      : "No students are assigned to this division yet."}
                  </div>
                )}
              </>
            )}
          ...
          {activeView === "roster" && (
            <>
                  ...
                  >
                    ...
                    <div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs">
                      <div className="rounded-lg bg-slate-50 p-2 dark:bg-slate-800">
                        <span className="block text-slate-500">{t("gpa")}</span>
                      ...
                      <div className="rounded-lg bg-slate-50 p-2 dark:bg-slate-800">
                        ...
                        <strong>{student.behaviorScore}</strong>
                      </div>
                      <div className="rounded-lg bg-slate-50 p-2 dark:bg-slate-800">
                        <span className="block text-slate-500">
                          {t("attendanceScore")}
                        </span>
                        <strong>{student.attendanceScore}</strong>
                      </div>
                    </div>
                    {canEditStudents && (
                      <div
                      ...
                      >
                        ...
                        <button
                          ...
                          className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700 dark:border-blue-900 dark:bg-blue-950/30 dark:text-blue-300"
                        >
                          {t("transferAction")}
                        </button>
                        {canDeleteStudents && (
                          <button
                            ...
                            className="rounded-lg border border-red-200 px-3 py-2 text-xs font-semibold text-red-600 dark:border-red-900 dark:text-red-300"
                          >
                            <Trash2 className="h-4 w-4" />
                          </button>
                        )}
                  ...
                  <table className="min-w-full text-sm">
                    <thead className="bg-slate-50 dark:bg-slate-800">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-start">{t("gpa")}</th>
                        <th className="px-4 py-3 text-start">
                          {t("parentPhone")}
                        </th>
                        <th className="px-4 py-3 text-start">
                        ...
                        >
                          {paginatedStudents.map((student, index) => (
                            <motion.tr
                              ...
                              className={`${canEditStudents ? "cursor-pointer" : ""} border-t border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50`}
                            >
                              {canEditStudents && (
                                ...
                                <div className="flex items-center gap-2">
                                  {canEditStudents && (
                                    <button
                                      ...
                                      className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
                                    >
              ...
              {activeView === "roster" && visibleStudents.length > 0 && (
                <div className="mt-6 flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900 sm:flex-row sm:items-center sm:justify-between">
                  <span className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-600 dark:bg-zinc-800 dark:text-slate-300">
                    {locale === "ar"
                      ? `عرض ${(currentPage - 1) * studentsPerPage + 1}–${Math.min(currentPage * studentsPerPage, visibleStudents.length)} من ${visibleStudents.length} طالب`
                      : `Showing ${(currentPage - 1) * studentsPerPage + 1}-${Math.min(currentPage * studentsPerPage, visibleStudents.length)} of ${visibleStudents.length} students`}
                  </span>
                  <div
                    className="flex items-center justify-between gap-1"
                    aria-label={
                      locale === "ar" ? "صفحات الطلاب" : "Student pages"
                    }
                  >
                    <button
                      type="button"
                      ...
                      className="rounded-lg border border-slate-200 p-2 text-slate-600 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
                    >
                    ...
                    {pageNumbers.map((page, index) =>
                      page === -1 ? (
                        ...
                        <button
                          ...
                          className={`min-w-9 rounded-lg px-2.5 py-2 text-sm font-semibold transition focus:outline-none focus:ring-2 focus:ring-emerald-500 ${currentPage === page ? "bg-emerald-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"}`}
                        >
                          {page}
                        </button>
                      ),
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-start transition-all duration-200 placeholder:text-slate-400 focus:border-emerald-500 focus:bg-white focus:ring-2 focus:ring-emerald-100 dark:border-slate-700 dark:bg-slate-950 dark:placeholder:text-slate-600 dark:focus:bg-slate-900 dark:focus:ring-emerald-900"
                  />
                </div>
                <div>
                  <label className="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-300">
                  ...
                  <input
                    ...
                    placeholder={t("exampleNationalId")}
                    className="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-start transition-all duration-200 placeholder:text-slate-400 focus:border-emerald-500 focus:bg-white focus:ring-2 focus:ring-emerald-100 dark:border-slate-700 dark:bg-slate-950 dark:placeholder:text-slate-600 dark:focus:bg-slate-900 dark:focus:ring-emerald-900"
                  />
                </div>
                <div>
                ...
                <button
                  type="submit"
                  disabled={
                    divisionOptions.length > 0 && !newStudent.divisionCode
                  }
                  className="mt-6 w-full rounded-lg bg-gradient-to-r from-emerald-600 to-emerald-700 px-4 py-3 text-sm font-semibold text-white transition-all duration-200 hover:from-emerald-700 hover:to-emerald-800 disabled:from-slate-300 disabled:to-slate-400 dark:disabled:from-slate-700 dark:disabled:to-slate-600"
                >
                  {t("saveStudent")}
                </button>
              </form>
        ...
        >
          <div dir="rtl">
            ...
            <p className="mt-3 text-sm text-slate-600 dark:text-slate-300">
              {t("importSummary")
                .replace("{count}", String(importResult.imported))
                .replace("{divisions}", String(importResult.divisionCount))}
            </p>
            <div className="mt-6 flex gap-3">
              ...
              <button
                type="button"
                onClick={() => setBulkEditing(true)}
                disabled={!importResult.students.length}
      ...
      {bulkEditing && bulkEditStudents.length > 0 && (
        <BulkStudentEditor
          students={bulkEditStudents}
          onClose={() => setBulkEditing(false)}
          onSaved={(updated) => {
            setStudents((current) =>
              current.map((student) =>
                updated.find((item) => item.id === student.id)
                  ? {
                      ...student,
                      ...updated.find((item) => item.id === student.id),
                    }
      ...
      <TransferModal
        ...
        divisions={divisionOptions.map((code) => ({ code }))}
      ...
      <ConfirmModal
        ...
        message={t("confirmDeleteStudent")}
