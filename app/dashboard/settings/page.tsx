...
export default function SettingsPage() {
  ...
  return <div className="space-y-6" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900"><div className="mb-5 flex items-center gap-3"><CalendarCheck className="h-5 w-5 text-blue-600" /><h2 className="text-xl font-bold text-slate-900 dark:text-white">{copy.attendance}</h2></div><label className="block"><span className="mb-2 block text-sm text-slate-600 dark:text-slate-300">{copy.defaultStatus}</span><AttendanceStatusSelect value={settings.defaultAttendance} onValueChange={(value) => updateSettings({ defaultAttendance: value as SavedSettings['defaultAttendance'] })} options={[{ value: 'UNMARKED', label: copy.unmarked }, { value: 'PRESENT', label: copy.present }]} english={english} /></label>{!isTeacher && <label className="mt-4 block"><span className="mb-2 block text-sm text-slate-600 dark:text-slate-300">{copy.lateTime}</span><div className="flex gap-2"><input type="time" value={settings.lateTime} onChange={(event) => updateSettings({ lateTime: event.target.value })} className="min-w-0 flex-1 rounded-lg border border-slate-300 bg-white px-3 py-2 dark:border-slate-700 dark:bg-slate-950" /><button type="button" onClick={applyLateTime} className="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white">{english ? 'Apply' : 'تطبيق'}</button></div><span className="mt-1 block text-xs text-slate-500">{copy.lateTimeHint}</span></label>}<label className="mt-4 flex items-center justify-between gap-3 text-sm text-slate-700 dark:text-slate-300"><span>{copy.notes}</span><Toggle checked={settings.attendanceNotes} onChange={(checked) => updateSettings({ attendanceNotes: checked })} /></label></section>
