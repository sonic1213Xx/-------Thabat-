...
export default function SettingsPage() {
  ...
  return <div className="space-y-6" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
    <div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-bold text-slate-900 dark:text-white">{copy.title}</h1><p className="text-slate-600 dark:text-slate-400">{copy.description}</p></div><button type="button" onClick={() => void saveAttendanceSettings()} disabled={settingsLoading} className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:cursor-wait disabled:opacity-60"><Save className="h-4 w-4" />{copy.saveSettings}</button></div>
    <div className="grid gap-5 lg:grid-cols-2">
      <section className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900"><div className="mb-5 flex items-center gap-3"><Building2 className="h-5 w-5 text-emerald-600" /><h2 className="text-xl font-bold text-slate-900 dark:text-white">{copy.school}</h2></div><label className="block"><span className="mb-2 block text-sm text-slate-600 dark:text-slate-300">{copy.schoolName}</span>{settingsLoading ? <div className="h-10 w-full animate-pulse rounded-lg bg-slate-200 dark:bg-slate-800" aria-label={copy.schoolName} /> : <input value={settings.schoolName} onChange={(event) => updateSettings({ schoolName: event.target.value })} className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-start dark:border-slate-700 dark:bg-slate-950" />}</label><p className="mt-3 rounded-lg bg-slate-50 p-3 text-sm text-slate-600 dark:bg-slate-800 dark:text-slate-300">{copy.identifier}</p></section>
      <section className="relative overflow-hidden rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900"><div className="mb-5 flex items-center gap-3"><CalendarCheck className="h-5 w-5 text-blue-600" /><h2 className="text-xl font-bold text-slate-900 dark:text-white">{copy.attendance}</h2></div>{settingsLoading && <div className="absolute inset-0 z-10 space-y-5 bg-white p-6 dark:bg-slate-900" aria-label={copy.attendance}><div className="h-7 w-2/3 animate-pulse rounded bg-slate-200 dark:bg-slate-800" /><div className="h-10 w-full animate-pulse rounded-lg bg-slate-200 dark:bg-slate-800" /><div className="h-10 w-full animate-pulse rounded-lg bg-slate-200 dark:bg-slate-800" /><div className="h-7 w-4/5 animate-pulse rounded bg-slate-200 dark:bg-slate-800" /></div>}<div className={settingsLoading ? 'invisible' : ''}><label className="block"><span className="mb-2 block text-sm text-slate-600 dark:text-slate-300">{copy.defaultStatus}</span><AttendanceStatusSelect value={settings.defaultAttendance} onValueChange={(value) => updateSettings({ defaultAttendance: value as SavedSettings['defaultAttendance'] })} options={[{ value: 'UNMARKED', label: copy.unmarked }, { value: 'PRESENT', label: copy.present }]} english={english} /></label>{!isTeacher && <label className="mt-4 block"><span className="mb-2 block text-sm text-slate-600 dark:text-slate-300">{copy.lateTime}</span><div><input type="time" value={settings.lateTime} onChange={(event) => updateSettings({ lateTime: event.target.value })} className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 dark:border-slate-700 dark:bg-slate-950" /></div><span className="mt-1 block text-xs text-slate-500">{copy.lateTimeHint}</span></label>}<label className="mt-4 flex items-center justify-between gap-3 text-sm text-slate-700 dark:text-slate-300"><span>{copy.notes}</span><Toggle checked={settings.attendanceNotes} onChange={(checked) => updateSettings({ attendanceNotes: checked })} /></label></div></section>
