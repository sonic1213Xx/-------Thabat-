  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (recipients.length) {
        ...
        await tx.transferNotification.createMany({ data: recipients.map((recipient) => ({ recipientId: recipient.id, createdBy: actor.id, fromDivision: fromDivisions.join(', '), toDivision, studentIdsJson: JSON.stringify(studentIds), studentSnapshotJson: JSON.stringify(studentSnapshot), gradeSnapshotJson: JSON.stringify(gradeSnapshot) })) })
      }
    })
