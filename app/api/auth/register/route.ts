...
export async function POST(request: NextRequest) {
  try {
    const body = await request.json() as { id?: string; name?: string; password?: string; divisions?: string[]; subjectsTaught?: string[] }
    ...
    return NextResponse.json({ data: { id: user.id, name: user.name, role: user.role, assigned_divisions: body.divisions ?? [], subjectsTaught: body.subjectsTaught ?? [] } }, { status: 201 })
  } catch (error) {
