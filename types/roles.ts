...
const principalPermissions: Permission[] = [
  ...
  'students:delete', 'attendance:approve', 'gradebooks:approve', 'gate_passes:approve', 'gate_passes:delete', 'reports:export', 'audit_log:delete',
]
...
export const ROLE_DEFINITIONS: readonly StaffRoleDefinition[] = [
  ...
  { key: 'VP_OPERATIONS', nameAr: 'وكيل الشؤون المدرسية', nameEn: 'VP Operations', descriptionAr: 'المرافق والموارد والطلبات الإدارية.', descriptionEn: 'Facilities, resources, and administrative requests.', permissions: [...readWrite('facilities'), ...readWrite('transportation'), 'gradebooks:read', 'reports:read', 'audit_log:read'], assignedDivisions: false },
  { key: 'VICE_PRINCIPAL', nameAr: 'وكيل شؤون الطلاب', nameEn: 'Vice Principal', descriptionAr: 'اسم قديم متوافق مع وكيل شؤون الطلاب.', descriptionEn: 'Legacy alias for Student Affairs.', permissions: [...readWrite('students'), ...readWrite('attendance'), ...readWrite('warnings'), ...readWrite('incidents'), ...readWrite('gate_passes'), 'gate_passes:approve', 'gate_passes:delete', 'gradebooks:read'], assignedDivisions: false },
