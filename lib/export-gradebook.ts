...
export async function exportGradebookToExcel(divisionName: string, studentsData: GradebookStudent[], customCategories: GradebookCustomCategory[] = [], customScores: Record<string, Record<string, number | null>> = {}, finalMaximum = 100, configuredFields: GradebookCustomCategory[] = [], period: GradebookPeriod = 'both') {
  ...
  worksheet.columns = [
    ...
    { key: 'name', width: 30 },
    { key: 'gpa', width: 18 },
    { key: 'taskPeriod1', width: 18 },
    { key: 'taskPeriod2', width: 18 },
    { key: 'examPeriod1', width: 18 },
    { key: 'examPeriod2', width: 18 },
    { key: 'finalExam', width: 18 },
    { key: 'total', width: 18 },
  ]
  ...
  const selectedFields = fields.filter((field) => period === 'both' || (period === 'period1' ? field.key.endsWith('Period1') || field.key === 'finalExam' : field.key.endsWith('Period2') || field.key === 'finalExam'))
  const exportHeaders = ['م', 'اسم الطالب', 'المعدل التراكمي', ...selectedFields.map((field) => `${field.label} (${field.max})`), ...customCategories.map((category) => `${category.label} (${category.max})`), `المجموع النهائي (${finalMaximum})`]
  ...
  studentsData.forEach((student, index) => {
    const row = worksheet.addRow([
      ...
      student.fullName,
      student.gpa ?? '',
      ...selectedFields.map((field) => student[field.key as keyof GradebookStudent] ?? ''),
      ...
      (() => {
        ...
        const fieldTotal = selectedFields.reduce((sum, field) => sum + Number(student[field.key as keyof GradebookStudent] ?? 0), 0)
...
export function exportGradebookToPdf(divisionName: string, studentsData: GradebookStudent[], fields: GradebookCustomCategory[], finalMaximum = 100, period: GradebookPeriod = 'both') {
  if (typeof window === 'undefined') return
  const selectedFields = fields.filter((field) => period === 'both' || (period === 'period1' ? field.key.endsWith('Period1') || field.key === 'finalExam' : field.key.endsWith('Period2') || field.key === 'finalExam'))
  const headers = ['م', 'اسم الطالب', 'المعدل التراكمي', ...selectedFields.map((field) => `${field.label} (${field.max})`), `المجموع النهائي (${finalMaximum})`]
  const rows = studentsData.map((student, index) => {
    ...
    return `<tr><td>${index + 1}</td><td>${escapeHtml(student.fullName)}</td><td>${student.gpa ?? '-'}</td>${selectedFields.map((field) => `<td>${student[field.key as keyof GradebookStudent] ?? ''}</td>`).join('')}<td><strong>${finalScore}</strong></td></tr>`
  }).join('')
  const printWindow = window.open('', '_blank', 'width=1100,height=800')
  if (!printWindow) return
  printWindow.document.write(`<html dir="rtl"><head><title>كشف درجات ${escapeHtml(divisionName)}</title><style>body{font-family:Arial,sans-serif;padding:24px;color:#111}h1{text-align:center;font-size:22px}table{width:100%;border-collapse:collapse;margin-top:20px}th,td{border:1px solid #9ca3af;padding:8px;text-align:center}th{background:#d9ead3;font-weight:700}td:nth-child(2){text-align:right}@media print{body{padding:0}}</style></head><body><h1>كشف درجات ${escapeHtml(divisionName)}</h1><table><thead><tr>${headers.map((header) => `<th>${escapeHtml(header)}</th>`).join('')}</tr></thead><tbody>${rows}</tbody></table><script>window.onload=()=>{window.print();window.onafterprint=()=>window.close()}</script></body></html>`)
...
export async function exportEmptyGradebookTemplatesToPdf(divisionCodes: string[]) {
  ...
  if (!printWindow) return
  printWindow.document.write(`<html dir="rtl"><head><title>قوالب كشوف الدرجات</title><style>body{font-family:Arial,sans-serif;padding:24px;color:#111}h1{text-align:center;color:#047857}h2{border-right:5px solid #047857;padding:8px 12px;background:#ecfdf5;page-break-after:avoid}section{page-break-after:always}section:last-child{page-break-after:auto}table{width:100%;border-collapse:collapse;margin-top:12px}th,td{border:1px solid #94a3b8;padding:8px;text-align:center;height:24px}th{background:#047857;color:#fff}td:nth-child(2){text-align:right}@media print{body{padding:0}}</style></head><body><h1>قوالب كشوف الدرجات</h1>${sections}<script>window.onload=()=>{window.print();window.onafterprint=()=>window.close()}</script></body></html>`)
  printWindow.document.close()
}
